...
> 方法：全量聚合决策统计 + 240 对分层抽样人工（AI 辅助）标注 + 碎片化（同事件签名拆分）探测。

## 1. 结论摘要

| 维度 | 结果 | 判定 |
|---|---|---|
| AI 合并决策精度 | approved 12/12 正确；declined 202/207 正确 | **决策层精准** |
...
- 样本：240 对（分层：≥95 / 70-95 / 55-70 / <55 / null 各 ~60 对）
- 全部样本经逐对阅读标题、摘要、事件签名判断
